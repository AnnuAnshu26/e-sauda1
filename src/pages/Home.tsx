...
import { ArrowRight, Lock, ShieldCheck, Truck, Camera, Sparkles, Mic } from 'lucide-react'
...
const features = [
  ...
  { icon: Camera, title: 'AR try-on & 360° spins', desc: 'See if the sofa fits before you buy. Orbit around a bike on your phone. No more grainy photos.' },
  ...
  { icon: Mic, title: 'Bolo, aur bech do', desc: 'Speak in Hindi, Tamil, Bangla — our voice engine fills the form. Listing done in 20 seconds.' },
]
    ...
    <div>
        ...
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-6 max-w-lg text-base text-ink/70">
              India's first marketplace where money sits in escrow, sellers are verified, and a
              rider shows up when you're ready. No brokers. No "OTP bhejo" scams. Just clean deals.
            </p>
            ...
            <div className="mt-10 flex gap-10">
              <div>
                <p className="font-display text-2xl font-semibold text-ink">₹42 Cr</p>
                <p className="text-xs uppercase tracking-wide text-ink/50">Vaulted last month</p>
              </div>
              <div>
                <p className="font-display text-2xl font-semibold text-ink">2.1 M</p>
                <p className="text-xs uppercase tracking-wide text-ink/50">Verified sellers</p>
              </div>
              <div>
                <p className="font-display text-2xl font-semibold text-ink">4.8★</p>
                <p className="text-xs uppercase tracking-wide text-ink/50">Buyer rating</p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-8">
          {categories.map((c) => (
            ...
            >
              ...
              <span className="text-xs text-ink/50">{(c.count / 1000).toFixed(0)}k live</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="bg-forest py-4 text-cream">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-10 gap-y-2 px-6 text-sm font-medium">
          ...
          <span className="flex items-center gap-2">★ Two-way accountability score</span>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-14">
        <div className="mb-6 flex items-end justify-between">
          <div>
            <h2 className="font-display text-2xl font-semibold">Fresh in your city</h2>
            <p className="mt-1 text-sm text-ink/60">Real people, real items. Every listing gated by our anti-bot fee.</p>
          </div>
      ...
      <section className="bg-cream-dark py-16">
        <div className="mx-auto max-w-3xl px-6 text-center">
          ...
          <p className="mt-4 text-ink/70">
            Every rupee sits in a regulated escrow ledger until the buyer physically inspects the
            item. Seller sees a giant green banner — not a WhatsApp screenshot — before handing
            anything over.
          </p>
        </div>
        <div className="mx-auto mt-10 grid max-w-6xl grid-cols-1 gap-4 px-6 sm:grid-cols-2 lg:grid-cols-4">
          {steps.map((s) => (
            <div key={s.n} className="rounded-xl2 bg-white p-6">
              <p className="font-display text-2xl font-semibold text-clay">{s.n}</p>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16">
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-2">
          <h2 className="font-display text-4xl font-semibold leading-tight">
            Built like OLX <span className="italic text-clay">wishes</span> it were built.
          </h2>
          <p className="text-ink/70">
            Every feature exists to fix one specific way older classifieds break. Look around.
          </p>
        </div>
        <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f) => (
            <div key={f.title} className="rounded-xl2 border border-black/5 bg-white p-6">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20">
        <div className="rounded-xl2 bg-gradient-to-br from-forest to-forest-light px-10 py-14 text-cream">
          <p className="font-display text-4xl font-semibold">Your first listing costs ₹1.</p>
          ...
          <p className="mt-4 max-w-xl text-cream/80">
            Join the marketplace built by neighbours, for neighbours. Fair fees, real people, and
            money that only moves when you're ready.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link to="/sell" className="flex items-center gap-2 rounded-full bg-clay px-6 py-3 text-sm font-semibold text-white hover:bg-clay-light">
              Start selling <ArrowRight size={16} />
            </Link>
            <Link to="/browse" className="rounded-full border border-cream/30 px-6 py-3 text-sm font-semibold text-cream hover:bg-white/10">
