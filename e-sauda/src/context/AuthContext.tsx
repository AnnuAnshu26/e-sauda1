...
interface AuthContextValue {
  ...
  // Mobile number + OTP, two-step: request an OTP by SMS, then verify it.
  // Requires a phone provider (e.g. Twilio/MSG91) to be configured in the
  // Supabase dashboard under Authentication -> Providers -> Phone — see
  // AUTH_SETUP.md for the exact steps, since that part can't be done from
  // application code.
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Step 1 of phone login: Supabase sends a one-time code by SMS through
  // whichever phone provider is configured for this project (Dashboard ->
  // Authentication -> Providers -> Phone). Reused for both sign-up and
  // sign-in -- Supabase's phone OTP flow creates the user on first verify if
  // one doesn't already exist, so the caller doesn't need to know in advance
  // whether this number has signed in before.
  async function requestPhoneOtp(phone: string) {
    const { error } = await supabase.auth.signInWithOtp({ phone })
    return { error: error ? error.message : null }
  }

  // Step 2: verifying the code the person received by SMS actually creates
  // the session (handled by supabase-js internally, same as signInWithPassword).
