    ...
    <header className="sticky top-0 z-40 border-b border-ink/10 bg-cream/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3 sm:gap-4 sm:px-6 sm:py-4">
        <Link to="/" className="flex shrink-0 items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-full border border-ink/15 font-display text-base italic text-ink">
            e
          </span>
          <span className="hidden font-display text-xl italic text-ink sm:inline">e-Sauda</span>
        </Link>

        {/* Full search bar: hidden below sm, where it doesn't fit alongside the logo,
            nav and auth controls. A compact icon-only trigger replaces it on mobile
            (below), routing to /browse where the same query can be typed full-width. */}
        <form
          onSubmit={onSearch}
          className="hidden max-w-xl flex-1 items-center gap-2 rounded-full border border-ink/10 bg-cream-dark/60 px-4 py-2 sm:flex"
        >
          ...
          <button
            type="submit"
            className="shrink-0 text-xs font-medium uppercase tracking-widest2 text-clay"
          >
            Search
          </button>
        </form>
        ...
        <Link
          to="/browse"
          className="flex shrink-0 items-center justify-center rounded-full border border-ink/10 p-2 text-ink/60 sm:hidden"
          aria-label="Search"
        >
          <Search size={17} />
        </Link>

        <nav className="hidden shrink-0 items-center gap-6 text-xs font-medium uppercase tracking-widest2 text-ink/60 md:flex">
        ...
        {user && (
          <div className="relative shrink-0" ref={notifRef}>
            ...
            >
              <Bell size={17} />
              {unreadCount > 0 && (
                <span className="absolute -right-0.5 -top-0.5 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-clay px-1 text-[10px] font-medium text-white">
                  {unreadCount > 9 ? '9+' : unreadCount}
                </span>
              )}
            </button>
            {notifOpen && (
              <div className="absolute right-0 mt-2 w-80 rounded-xl2 border border-ink/10 bg-cream p-2 shadow-none">
                <div className="flex items-center justify-between border-b border-ink/10 p-2">
        ...
        {!user ? (
          <div className="flex shrink-0 items-center gap-2">
            <Link to="/login" className="rounded-full px-4 py-2 text-sm font-medium text-ink/70 hover:text-ink">
              Log in
            </Link>
            <Link to="/signup" className="rounded-full bg-forest px-4 py-2 text-sm font-medium text-cream hover:bg-forest-light">
              Sign up
            </Link>
          </div>
        ) : (
          <>
            <div className="relative shrink-0" ref={menuRef}>
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-full border border-ink/15 font-display text-base italic text-ink"
              >
                {initial}
              </button>
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-64 rounded-xl2 border border-ink/10 bg-cream p-2 shadow-none">
                  <div className="flex items-center gap-3 border-b border-ink/10 p-3">
                    <span className="flex h-9 w-9 items-center justify-center rounded-full border border-ink/15 font-display text-base italic text-ink">
                      {initial}
                    </span>
                    <div>
                      ...
                      <p className="text-xs text-ink/50">Trust {profile?.trust_score ?? 50}</p>
                    </div>
                  </div>
            ...
            <Link
              to="/sell"
              className="flex shrink-0 items-center gap-1 rounded-full bg-forest px-4 py-2 text-sm font-medium text-cream hover:bg-forest-light"
            >
              <Plus size={16} /> Sell
            </Link>
          </>
