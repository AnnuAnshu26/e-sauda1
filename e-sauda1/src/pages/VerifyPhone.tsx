...
export default function VerifyPhone() {
  ...
  // Auto-send once as soon as we know the phone number to verify. If the profile has
  // no phone number at all (e.g. this row predates the phone_number trigger fix, or
  // the signup upsert genuinely failed), surface that instead of sitting on a blank
  // "we sent a code" screen with nothing actually sent.
  useEffect(() => {
    if (!profile) return
    if (profile.phone_number) {
      if (!sentOnce && !sending) sendCode()
    } else {
      setError(
        "We don't have a mobile number on file for your account. Please sign out and sign up again, or contact support.",
      )
    }
